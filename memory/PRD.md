...
## Phase 4 — Workspace + Document Questions + ATO Rules Engine (Feb 28, 2026 — FINAL build phase)
- **FIX A (inbox count)**: `tax_return_summary` now counts docs where `category="00 Inbox"` OR `needs_assignment=True` OR `accountant_review_required + status="Accountant review"` — matches what the Phase 2 pipeline actually writes.
- **FIX B (naming)**: scrubbed `Gavin Christie` from CreateTaxReturn placeholder, Phase 1 test fixture, and the TaxReturn entity_name comment. Going forward, taxpayer names live only in DB `entity_name`.
- **FIX C (profile auto-match)**: in `missing_evidence.check_and_update_missing_evidence`, added a second matching pass that finds profile-generated items (`generated_by="profile"`) belonging to the document's `tax_return_id` whose `item_needed` contains keywords from a `DOCTYPE_TO_ITEM_HINTS` table keyed by code-triage `doc_type`. Cautious match — flips to **`Possible Match`** (not Received) and writes `matched_document_id`/`match_confidence="Likely"`/`match_reason`. Live E2E verified ✅
- **ATO Rules Engine** (`backend/tax_rules_engine.py` + `backend/tax_rules.json`): year-versioned rates editable as JSON. Helpers: `calculate_car_cents_per_km`, `calculate_wfh_fixed_rate`, `calculate_phone_internet_apportionment`, `work_expense_evidence_threshold`. Every output labelled DRAFT + `accountant_review_required` flag. Routes: `GET /api/tax-rules/{rule_key}/{tax_year}`, `POST /api/tax-rules/calculate/{car-cents-per-km|wfh-fixed-rate|phone-internet}`, `GET /api/tax-rules/threshold/work-expenses/{tax_year}`. Verified: FY2025 / 3000km → **draft_amount $2,640.00**; cap at 5,000km enforced.
- **Document Questions** (`backend/document_questions.json`): catalogue indexed by `doc_type`. Upload pipeline now attaches a `questions[]` array per document (no AI). Routes: `PATCH /api/documents/{id}/questions/{key}` (saves answer + marks `user_confirmed=true`), `GET /api/tax-returns/{id}/unanswered-questions` (aggregator). Live verified ✅
- **`document_type` now persisted** on the document row (was only inside the in-memory `analysis` blob). Required for FIX C matching and aggregator endpoint.
- **Workspace page** `/tax-returns/{id}` (`TaxReturnWorkspace.jsx`): breadcrumb, big title with status pill + completion %, 4 stat cards (Documents / Needs Review / Outstanding / Open Questions), Questions block with inline answer forms (yes/no, single_select, percent, text), Missing Evidence section with priority badges.
- **Tax Returns list** `/tax-returns` (`TaxReturnsList.jsx`): all returns sorted newest-first, status-coloured badges, click-through to workspace.
- **Sidebar**: added "Tax Returns" entry (`FileBarChart2` icon) above the existing "+ New Tax Return" link.
- **Tests** (`backend/tests/test_phase4_workspace.py`, 12 cases): rule engine math (car CPK, 5000km cap, WFH, phone apportionment thresholds), unknown-rule 404, summary endpoint shape, document-question 404, regressions on `/documents` and `/missing-evidence`, **plus an end-to-end FIX C test** that seeds a profile item, uploads a real synergy PDF through `/api/uploads/bulk`, waits for the background classifier, and asserts the item flipped to `Possible Match` with `matched_document_id` populated.
- **Full backend suite**: **172 passed / 5 skipped** (+12 new Phase 4 tests; no regressions). `grep -r "Gavin Christie" backend frontend/src` returns zero hits.
- **Post-MVP / deferred**: bank-transaction-context classification, fuzzy filename duplicate detection, merchant memory, "AI Review Selected" Claude deep-review, per-return Drive folder structure, Trust/Sole-Trader question pathways beyond placeholders, renaming `properties` collection.
- **Files touched in Phase 4**: NEW: `backend/tax_rules_engine.py`, `backend/tax_rules.json`, `backend/document_questions.json`, `backend/tests/test_phase4_workspace.py`, `frontend/src/pages/TaxReturnWorkspace.jsx`, `frontend/src/pages/TaxReturnsList.jsx`. MODIFIED: `backend/server.py` (FIX A query, FIX B comment, Phase 4 routes + `Any` typing import), `backend/upload_pipeline.py` (attach `questions[]` + persist `document_type`), `backend/missing_evidence.py` (FIX C second pass), `backend/tests/test_phase1_tax_returns.py` (FIX B), `frontend/src/pages/CreateTaxReturn.jsx` (FIX B placeholder), `frontend/src/App.js` (+2 routes), `frontend/src/components/Layout.jsx` (+1 sidebar link). Forbidden files (`ai_classifier.py`, `extraction.py`, AI / Drive paths) untouched.
