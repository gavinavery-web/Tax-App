...
export default function UploadDialog({ open, onOpenChange, reference, onUploaded, initialFile, initialMeta, autoSubmit }) {
  ...
  useEffect(() => {
    if (open && initialFile) {
      ...
      setManualFolder(m.manual_drive_folder || "");
      setManualLink(m.manual_drive_link || "");
    }
    if (!open) {
      setFile(null); setName(""); setTaxYear("FY2024"); setCategory("ATO");
      setNotes(""); setReview("No"); setManualFolder(""); setManualLink("");
    }
  ...
  const doUpload = async (fileArg, metaArg) => {
    ...
    const m = metaArg || { name, tax_year: taxYear, category, notes, accountant_review: review, manual_drive_folder: manualFolder, manual_drive_link: manualLink };
    setUploading(true);
    try {
      ...
      fd.append("manual_drive_folder", m.manual_drive_folder || "");
      fd.append("manual_drive_link", m.manual_drive_link || "");
      const res = await api.post("/documents", fd, { headers: { "Content-Type": "multipart/form-data" } });
      const where = res.data.storage === "drive" ? "Google Drive" : "app storage (fallback)";
  ...
  useEffect(() => {
    if (open && autoSubmit && initialFile) doUpload(initialFile, initialMeta);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl rounded-sm max-h-[92vh] overflow-y-auto" data-testid="upload-dialog">
        <DialogHeader>
        ...
        {driveConnected === false && (
          <div className="bg-amber-50 border border-amber-200 rounded-sm p-2.5 flex items-start gap-2" data-testid="upload-fallback-notice">
            <CloudOff className="w-4 h-4 text-amber-700 mt-0.5 shrink-0" />
            <div className="text-xs text-amber-900 leading-relaxed">
              Drive is not connected — this document will be stored on the app server. Optionally paste the Google Drive folder name and link where you've placed the file manually.
            </div>
          </div>
        )}
        ...
        <div className="space-y-3">
          <div>
            ...
            <Input type="file" onChange={onFile} accept=".pdf,.png,.jpg,.jpeg,.webp,.heic,.heif,.xls,.xlsx,.csv,.doc,.docx,.txt" className="rounded-sm mt-1" data-testid="upload-file-input" disabled={!!autoSubmit} />
          ...
          {driveConnected === false && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-xs uppercase tracking-wider text-zinc-500">Manual Drive folder (optional)</Label>
                <Input value={manualFolder} onChange={(e) => setManualFolder(e.target.value)} placeholder="e.g. Tax Evidence Vault/01 ATO" className="rounded-sm mt-1 mono text-xs" data-testid="upload-manual-folder" />
              </div>
              <div>
                <Label className="text-xs uppercase tracking-wider text-zinc-500">Manual Drive link (optional)</Label>
                <Input value={manualLink} onChange={(e) => setManualLink(e.target.value)} placeholder="https://drive.google.com/..." className="rounded-sm mt-1 mono text-xs" data-testid="upload-manual-link" />
              </div>
            </div>
          )}
